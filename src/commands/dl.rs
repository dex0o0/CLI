...
pub async fn dl_read_file(file: PathBuf) -> Result<()> {
    let links = fs::read_to_string(file).expect("Error can't read file");
    for url in links.lines() {
        ...
        download(&link.string as &str)
            .await
            .expect("Error to download");
        println!("\n");
    }
    Ok(())
}
...
impl DownloadChecker {
    pub async fn check_all(url: &str, path: &PathBuf) -> bool {
        ...
        matches!(Self::handle_redirect(&client, url).await, Ok(_e))
    }
    ...
    async fn check_space(url: &str, path: &Path) -> bool {
        let client = client_build();
