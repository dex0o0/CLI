...
impl Sysinfo {
    ...
    pub fn auto_fill(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.hyperthreading_enabled = if self.total_processor > self.physical_cores {
            true
        } else {
            false
        };
    ...
    pub fn data_vec(&self) -> Vec<(String, String)> {
        let data = vec![
            ...
            (
                "memory".to_string(),
                format!("{:.2}GB", self.memory_size_as_mb.clone() as f32 / 1000.0).to_string(),
            ),
            ...
            (
                ...
                format!("{:.2}GB", self.gpu_memory_as_mb.clone() as f32 / 1000.0).to_string(),
            ),
            (
