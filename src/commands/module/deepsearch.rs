...
impl SearchOPT {
    ...
    pub fn add_block<T: ToString>(&mut self, blockpath: T) {
        self.blacklist.push(blockpath.to_string());
    }
}

fn is_blacklisted(path: &Path, blk: &SearchOPT) -> bool {
    blk.blacklist
        .iter()
...
fn deep_search_depth(
    ...
    if check_err(&path, &current_depth, blk) {
        return;
    }
        ...
        for (i, path) in paths.iter().enumerate() {
            ...
            if target.is_empty() || name.contains(target) {
                if path.is_dir() {
                    ...
                    deep_search_depth(path.clone(), target, current_depth - 1, initial_depth, blk);
                } else {
...
fn check_err(path: &PathBuf, depth: &usize, blk: &SearchOPT) -> bool {
    let mut bool = false;
    if *depth == 0 {
        bool = true;
    }
    if is_blacklisted(path, blk) {
        bool = true;
    }
